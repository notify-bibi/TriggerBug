...
std::unique_ptr<GenLLVM>& GenLLVM::getGenLLVM()
{
	return theGenLLVM;
}
...
#else
		gep_name = regOff2name(access_bytes * off_u64);
#endif
	}
...
void GenLLVM::store(Value* addr_v, Value* data_v)
{
	...
	si->setAlignment(llvm::assumeAligned(8));
}
...
void GenLLVM::mkFuncTy(unsigned N)
{
...
Value* GenLLVM::to16x8i(Value* v) const
{
	return builder->CreateBitCast(
		v,
		VectorType::get(
			Type::getInt8Ty(getGlobalContext()), 16, SCALABLE));
}
...
Value* GenLLVM::to32x8i(Value* v) const
{
	return builder->CreateBitCast(
		v,
		VectorType::get(
			Type::getInt8Ty(getGlobalContext()), 32, SCALABLE));
}
...
Type* GenLLVM::getGuestTy(void)
{
	...
	auto			i128ty = VectorType::get(i16ty, 8, SCALABLE);
	auto			i256ty = VectorType::get(i16ty, 16, SCALABLE);
#if 0
...
#endif
	guestCtxTy = StructType::create(gctx, types, "guestCtxTy");
	return guestCtxTy;
}
