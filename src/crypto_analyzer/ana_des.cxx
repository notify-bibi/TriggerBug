#define _ENABLE_EXTENDED_ALIGNED_STORAGE
#include "crypto_analyzer/ana_des.h"


using namespace z3;


namespace aes {
    alignas(64)
        const uint8_t SE[256] = {
           
    };


    inline constexpr uint8_t xtime(uint8_t s) { return static_cast<uint8_t>(s << 1) ^ ((s >> 7) * 0x1B); }
    inline constexpr uint8_t xtime4(uint8_t s) { return xtime(xtime(s)); }
    inline constexpr uint8_t xtime8(uint8_t s) { return xtime(xtime(xtime(s))); }

    inline constexpr uint8_t xtime3(uint8_t s) { return xtime(s) ^ s; }
    inline constexpr uint8_t xtime9(uint8_t s) { return xtime8(s) ^ s; }
    inline constexpr uint8_t xtime11(uint8_t s) { return xtime8(s) ^ xtime(s) ^ s; }
    inline constexpr uint8_t xtime13(uint8_t s) { return xtime8(s) ^ xtime4(s) ^ s; }
    inline constexpr uint8_t xtime14(uint8_t s) { return xtime8(s) ^ xtime4(s) ^ xtime(s); }

    constexpr uint32_t make_uint32(
        uint8_t 	i0,
        uint8_t 	i1,
        uint8_t 	i2,
        uint8_t 	i3
    ) {
        return ((static_cast<uint32_t>(i0) << 24) |
            (static_cast<uint32_t>(i1) << 16) |
            (static_cast<uint32_t>(i2) << 8) |
            (static_cast<uint32_t>(i3)));
    }

    template<typename T> 
    constexpr T rotate_right(T input, size_t rot) __attribute__((always_inline))
    {
        rot %= 8 * sizeof(T);
        return static_cast<T>((input >> rot) | (input << (8 * sizeof(T) - rot)));
    }

    const uint32_t* AES_TE()
    {
        class TE_Table final
        {
        public:
            TE_Table()
            {
                uint32_t* TE = reinterpret_cast<uint32_t*>(&data);
                for (size_t i = 0; i != 256; ++i)
                {
                    const uint8_t s = SE[i];
                    const uint32_t x = make_uint32(xtime(s), s, s, xtime3(s));
                    TE[i] = x;
                    TE[i + 256] = rotate_right(x, 8);
                    TE[i + 512] = rotate_right(x, 16);
                    TE[i + 768] = rotate_right(x, 24);
                }
            }
            const uint32_t* ptr() const { return reinterpret_cast<const uint32_t*>(&data); }
        private:
            std::aligned_storage<1024 * sizeof(uint32_t), 64>::type data;
        };

        static TE_Table table;
        return table.ptr();
    }





    expr item(expr const& idx)
    {
        context& ctx = idx.ctx();
        expr r = ctx.bv_val(0x63, 8);
        for (int i = 1; i < 256; i++) {
            expr d = idx == i;
            r = ite(d, ctx.bv_val(SE[i], 8), r);
        }
        return r;
    }


    static inline z3::expr xtime(z3::expr const& s) { return (s << 1) ^ ((z3::lshr(s, 7)) * 0x1B); }
    static inline z3::expr xtime3(z3::expr const& s) { return xtime(s) ^ s; }

    expr TE0(const expr& idx) {
        expr s = item(idx);
        return concat(concat(concat(xtime(s), (s)), (s)), xtime3(s));
    }

    expr TE1(const expr& idx) {
        expr s = item(idx);
        return concat(concat(concat(xtime3(s), xtime(s)), (s)), (s));
    }

    expr TE2(const expr& idx) {
        expr s = item(idx);
        return concat(concat(concat((s), xtime3(s)), xtime(s)), (s));
    }

    expr TE3(const expr& idx) {
        expr s = item(idx);
        return concat(concat(concat((s), (s)), xtime3(s)), xtime(s));
    }
}
using namespace aes;

ana_decl des_check_ro0(UInt idx, uint32_t value) {
    if (idx >= 256) return (ana_decl)-1;
    
    if (AES_TE()[idx] == value) 
        return TE0;
    
    return nullptr;
}

ana_decl des_check_ro1(UInt idx, uint32_t value) {
    if (idx >= 256) return (ana_decl)-1;

    if (AES_TE()[256 + idx] == value)
        return TE1;

    return nullptr;
}

ana_decl des_check_ro2(UInt idx, uint32_t value) {
    if (idx >= 256) return (ana_decl)-1;

    if (AES_TE()[512 + idx] == value)
        return TE2;

    return nullptr;
}

ana_decl des_check_ro3(UInt idx, uint32_t value) {
    if (idx >= 256) return (ana_decl)-1;

    if (AES_TE()[768 + idx] == value)
        return TE3;

    return nullptr;
}