   ...
   */
   ...
   /* And what we want to change it to is either:
      ...
      (2) even if the short form is applicable, once every (say)
          1024 times use the long long form anyway, so as to maintain
          verifiability
   */
                                   ...
                                   const void* disp_cp_chain_me )
{
   ...
   */
   ...
   if (is_imm32_to_ireg_EXACTLY2(
       ...
       && p[2] == 0xE12FFF1C) {
      valid = True; /* it's the long long form */
      if (0)
         vex_printf("QQQ unchainXDirect_ARM: found long long form\n");
   } else
