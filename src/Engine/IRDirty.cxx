...
public:
    ...
    //FORK
    DMEM(z3::solver& so, TRcontext& ctx, DMEM& father_mem, Bool _need_record): MEM(so, ctx, father_mem, _need_record),
        m_stack_reservve_size(0),
        m_stack_addr(0),
        m_guest_regs_map_addr(0)
    {
...
public:
    //dirty call
    template<typename ADDR>
    DState(Vex_Info const& info, TRcontext& ctx, TRsolver&solver, MEM<ADDR>& memory) :
        m_base(nullptr),
        _m_z3_ctx(),
        _m_z3_solv(ctx),
        m_ctx(ctx),
    ...
    virtual bool  StateCompression(DState const& next) { return true; }
    virtual void  StateCompressMkSymbol(DState const& newState) {  }; 
