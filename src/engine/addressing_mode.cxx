...
TR::addressingMode<HWord>::addressingMode(const expr& e)
    :
    m_ctx(e.ctx()),
    m_symbolic(e),
    m_offset(m_ctx),
...
template<typename HWord>
TR::addressingMode<HWord>::iterator::iterator(TR::addressingMode<HWord>& am) :
    m_sym_mask(am.m_sym_mask),
    m_or_mask(am.m_or_mask),
    tmp_bit_blast((HWord)0),
    tmp_target_bit_blast((HWord)0),
    m_sym_ml_n(0),
    m_sign_ml_n(0)
{
    int N;
    ...
    if (ctzll(N, m_sym_mask)) {
        m_sym_ml[0] = shift_mask{ (UChar)N,((HWord)1) << N };
        m_sym_ml_n = 1;
        _pcur = N;
        tmp_target_bit_blast = ((HWord)1);
        nb = 1;

        for (; ; ) {
            if (ctzll(N, m_sym_mask & fastMaskI1Reverse(_pcur))) {
                if (N == _pcur + 1) {
                    m_sym_ml[m_sym_ml_n - 1].mask |= ((HWord)1) << N;
                }
                else {
                    m_sym_ml[m_sym_ml_n - 1] = shift_mask{ (UChar)N,((HWord)1) << N };
                    m_sym_ml_n++;
                }
                tmp_target_bit_blast |= ((HWord)1) << (nb++);
                _pcur = N;
            }
            else {
                break;
...
//a=b+c+d+e...+z -> b c d e
template<typename HWord>
void TR::addressingMode<HWord>::_offset2opAdd(std::deque<expr>& ret, expr const& _e)
{
    expr e = _e;
    context& c = e.ctx();
...
template<typename HWord>
bool TR::addressingMode<HWord>::_check_add_no_overflow(expr const& e1, expr const& e2) {
...
template<typename HWord>
bool TR::addressingMode<HWord>::ast2baseAoffset()
{
...
template<typename HWord>
bool TR::addressingMode<HWord>::offset_bit_blast()
{
    ...
    for (UInt idx = 0; idx < size; idx++) {
        ...
        if (s.sym_ast) {
            ...
            if (!exist) {
                vec.emplace_back(sbit_struct_r{ s  , ((HWord)1) << idx, 1 });
            };
        }
        else {
            m_or_mask = m_or_mask | ((HWord)s.rbit << idx);
        }
    }
...
template<typename HWord>
void TR::addressingMode<HWord>::print()
{
...
template<typename HWord>
expr TR::addressingMode<HWord>::_ast2base(expr& base,
    expr const& e,
...
template<typename HWord>
typename TR::addressingMode<HWord>::sbit_struct TR::addressingMode<HWord>::_check_is_extract(expr const& _e, UInt _idx) {
    context& c = _e.ctx();
    UInt idx = _idx;
    expr e = _e;
    //std::cout << e;
