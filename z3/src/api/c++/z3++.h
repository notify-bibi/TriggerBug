...
namespace z3 {
    ...
    */
    class z3exception : public std::exception {
        std::string m_msg;
    public:
        z3exception(char const * msg):m_msg(msg) {}
        virtual ~z3exception() throw() {}
        char const * msg() const { return m_msg.c_str(); }
        char const * what() const throw() { return m_msg.c_str(); }
        friend std::ostream & operator<<(std::ostream & out, z3exception const & e);
    };
    inline std::ostream & operator<<(std::ostream & out, z3exception const & e) { out << e.msg(); return out; }
        ...
        */
        Z3_error_code check_error() const {
            ...
            if (e != Z3_OK && enable_exceptions())
                Z3_THROW(z3exception(Z3_get_error_msg(m_ctx, e)));
            return e;
        }
        ...
        */
        int get_numeral_int() const {
            ...
            if (!is_numeral_i(result)) {
                ...
                Z3_THROW(z3exception("numeral does not fit in machine int"));
            }
            return result;
        }
        ...
        */
        unsigned get_numeral_uint() const {
            ...
            if (!is_numeral_u(result)) {
                assert(ctx().enable_exceptions());
                if (!ctx().enable_exceptions()) return 0;
                Z3_THROW(z3exception("numeral does not fit in machine uint"));
            }
            return result;
        }
        ...
        */
        int64_t get_numeral_int64() const {
            ...
            if (!is_numeral_i64(result)) {
                assert(ctx().enable_exceptions());
                if (!ctx().enable_exceptions()) return 0;
                Z3_THROW(z3exception("numeral does not fit in machine int64_t"));
            }
            return result;
        }
        ...
        */
        uint64_t get_numeral_uint64() const {
            ...
            if (!is_numeral_u64(result)) {
                assert(ctx().enable_exceptions());
                if (!ctx().enable_exceptions()) return 0;
                Z3_THROW(z3exception("numeral does not fit in machine uint64_t"));
            }
            return result;
        }
...
#undef _Z3_MK_UN_
    ...
    */
    ...
    public:
        ...
        expr eval(expr const & n, bool model_completion=false) const {
            ...
            if (status == false && ctx().enable_exceptions())
                Z3_THROW(z3exception("failed to evaluate expression"));
            return expr(ctx(), r);
        }
    ...
    inline tactic par_or(unsigned n, tactic const* tactics) {
        if (n == 0) {
            Z3_THROW(z3exception("a non-zero number of tactics need to be passed to par_or"));
        }
