...
def convert_f80le_to_f64le (f80):
    ...
    """
    if (bexp == 0x7FFF):
        isInf = (f80[7] & 0x7F) == 0 and f80[6] == 0 and f80[5] == 0 and f80[4] == 0 and f80[3] == 0 and f80[2] == 0 and f80[1] == 0 and f80[0] == 0
...
def FOA2VA(segment, FOA):
    segments = segment.split("\n")
    for segstr in segments:
        grammar = re.match("name:(?P<name>[:.=+\(\)_\w \[\]]*)address:(?P<address>[0-9a-f ]*) data offset:(?P<prd>[0-9a-f ]*) length:(?P<length>[0-9a-f ]*)", segstr)
        if (grammar):
            seg = grammar.groupdict()
            prd = int(seg["prd"],16)
            length = int(seg["length"],16)
            address = int(seg["address"],16)
            if(FOA>=prd and FOA<=length+prd):
                return FOA - prd + address
        else:
            print("unknow", grammar)
        ...
        for regAddress in regs:
            ...
            try:
                if size==1:
                    ...
                    db_data=struct.pack(up("<QQQQ"),INT&0xffffffffffffffff,(INT>>64)&0xffffffffffffffff,(INT>>128)&0xffffffffffffffff,INT>>192)
                else:
                    continue
