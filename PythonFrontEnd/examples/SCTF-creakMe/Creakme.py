# coding:utf-8
import archinfo
import builtins

#builtins.develop_mode = True
builtins.TriggerBug_Mode = 32
import ctypes
import TriggerBug
import TriggerBug.z3 as z3

top_state = TriggerBug.TopState(
    file_name = r'./ckm.xml',
    need_record = True,
    # 在需要合并(compress)state时，必须要保证被compress 的state对象的need_record=True，否则会报错，因为need_record=False的state是不会记录子state生命周期中所产生的修改,继而无法合并
    oep = 0,
    Ijk_unsupport_call = None,  # if set,Ir jump kind call what I not support will call this func
    Ijk_hook_call = None  # if set, all the Ir jump kind call will call this func
)
flag=[]
for i in range(27):
    k = z3.BitVec("flag%d" % i, 8, top_state.ctx)
    flag.append(k)
    top_state.add(z3.And(z3.UGT(k, 29), z3.ULE(k, 128), top_state.ctx), True)
    top_state.mem_w(top_state.ebp - 0x40 + i, k, 1)

top_state.mem_w(top_state.ebp - 0x40 + 27, 0, 1)
top_state.mem_w(0x4025BA, 0x8d, 1)


SE = [
    ]

def item(ctx, expr, base):
    r = z3.BitVecVal(0x63, 8, ctx)
    for i in range(1, len(SE)):
        r = z3.If(expr == base + i * 4, SE[i], r)
    return r


def xtime(s):
    return (s << 1) ^ ((z3.LShR(s, 7)) * 0x1B)


def xtime3(s):
    return xtime(s) ^ s


def dword_4062E0(_state, maddr):
    print("dword_4062E0")
    s = item(_state.ctx, maddr, 0x4062E0)
    return z3.Concat((s), (s), xtime3(s), xtime(s))


def dword_4059E0(_state, maddr):
    s = item(_state.ctx, maddr, 0x4059E0)
    return z3.Concat(xtime(s), (s), (s), xtime3(s))


def dword_4055E0(_state, maddr):
    s = item(_state.ctx, maddr, 0x4055E0)
    return z3.Concat(xtime3(s), xtime(s), (s), (s))


def dword_4051E0(_state, maddr):
    s = item(_state.ctx, maddr, 0x4051E0)
    return z3.Concat((s), xtime3(s), xtime(s), (s))


TriggerBug.idx2table_add(0x4062E0, dword_4062E0)
TriggerBug.idx2table_add(0x4059E0, dword_4059E0)
TriggerBug.idx2table_add(0x4055E0, dword_4055E0)
TriggerBug.idx2table_add(0x4051E0, dword_4051E0)


def evalvalue(_state = TriggerBug.State):
    global flag
    for i in range(11):
        _state.add(_state.mem_r(_state.ecx + i*4, 4)== _state.mem_r(_state.edi + i*4, 4), True)
    print("check")
    if _state.solver.check()==z3.sat:
        m = _state.solver.model()
        print(m)
        flagv =  m.eval(z3.Concat(*flag)).as_long()
        print(flagv.to_bytes(28, byteorder='big'))
    return TriggerBug.Death

top_state.hook_add(0x0402630, evalvalue)

top_state.go()
top_state.wait()
print(top_state)
